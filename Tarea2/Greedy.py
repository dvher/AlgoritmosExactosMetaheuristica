...
def greedy_determinista(num_uavs: int, uavs: List[List[int]], t_espera: List[List[int]]) -> Tuple[int, List[int]]:
    ...
    for n, i in enumerate(orden):
        tiempo_aterrizaje = max(uavs[i][1], tiempos_aterrizaje[orden[n - 1]] + t_espera[orden[n - 1]][i] if n > 0 else 0)
        ...
        elif tiempo_aterrizaje > uavs[i][1] and tiempo_aterrizaje <= uavs[i][2]:
...
def greedy_estocastico(num_uavs: int, uavs: List[List[int]], t_espera: List[List[int]], seed: int) -> Tuple[int, List[int]]:
    ...
    for n, i in enumerate(orden):
        t_sig_aterrizaje = max(uavs[i][0], tiempos_aterrizaje[orden[n - 1]] + t_espera[orden[n - 1]][i] if n > 0 else 0)
        ...
        if t_sig_aterrizaje < uavs[i][2]:
            ...
            tiempos_aterrizaje[i] = tiempo_aterrizaje
            diferencia = 0
        porcentaje = diferencia * 0.05 # Mientras mayor sea el porcentaje mas dispersos los tiempos que tomará, y más probabilidad de caer en soluciones infactibles
